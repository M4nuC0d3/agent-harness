...
Instructions for AI coding agents working in this repo. This is the **canonical** file: `CLAUDE.md` and `GEMINI.md` import it rather than duplicating it.
...
Delegate each subtask to one of these roles. The default loop per subtask is `researcher` (if needed) → `implementer` → `evaluator`; on FAIL, return the feedback to `implementer`, max 2 revisions, then escalate to the human.
...
The full prompt for each role follows. If your tool supports sub-agents, it will have its own copy (e.g. `.claude/agents/`, `.vibe/agents/`); otherwise, adopt the role inline when you take on that subtask.
